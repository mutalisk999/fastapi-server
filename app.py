        ...
        if hasattr(request.state, 'db'):
            try:
                ...
                if hasattr(request.state.db, 'close'):
                    ...
                    if hasattr(request.state.db, 'max_connections') and request.state.db.max_connections > 1:
...
class Application(object):
    ...
    def create_app(config_name: str, config_pass: str):
        ...
        init_logger()
        logger.info(f"Logger initialized with config: file={Application.setting.LOG_FILE_NAME}, level={Application.setting.LOG_LEVEL}")
